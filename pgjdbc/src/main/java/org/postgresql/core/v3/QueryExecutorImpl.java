...
public class QueryExecutorImpl extends QueryExecutorBase {
   ...
   * send has already put part of a message on the wire, and neither {@code startCopy} nor
   * {@code endCopy} can be retried, since that would send the query or CopyDone a second time.</p>
   */
  ...
  public void cancelCopy(CopyOperationImpl op) throws SQLException {
    ...
    try {
      ...
      // Cancelling ends the copy either way, so the lock has to go back. Otherwise future
      // operations, rather than failing due to the broken connection, would hang waiting for it.
      try (ResourceLock ignore = lock.obtain()) {
        if (hasLock(op)) {
          unlock(op);
        }
